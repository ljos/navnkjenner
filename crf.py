...
def crf_unary_score(tag_indices, sequence_lengths, inputs):
  ...
  unary_scores = embedding_ops.embedding_lookup(
    flattened_inputs,
    flattened_tag_indices
  )

  unary_scores = array_ops.reshape(unary_scores, [batch_size, max_seq_len])
...
class CrfDecodeBackwardRnnCell(rnn_cell.RNNCell):
  ...
  def __call__(self, inputs, state, scope=None):
    ...
    new_tags = array_ops.expand_dims(
        array_ops.gather_nd(inputs, indices),             # [B]
        axis=-1)                                              # [B, 1]
...
def crf_decode(potentials, transition_params, sequence_length):
  ...
  def _multi_seq_fn():
    ...
    backpointers, last_score = rnn.dynamic_rnn(  # [B, T - 1, O], [B, O]
        ...
        time_major=False,
        dtype=dtypes.int32)
    backpointers = array_ops.reverse_sequence(  # [B, T - 1, O]
    ...
    decode_tags = array_ops.concat([initial_state, decode_tags],   # [B, T]
                                   axis=1)
    decode_tags = array_ops.reverse_sequence(  # [B, T]
